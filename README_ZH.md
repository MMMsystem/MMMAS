...
MMMAS（孟德尔错配矩阵分析系统）是一款基于布尔孟德尔排除法则的种质库预筛工具，核心创新在于将成对排除逻辑拓展为 N×N对称错配矩阵，提出 Mmin（最小错配数）、Mavg（平均错配数）和 Mzmp（零错配伙伴数）三项核心指标，并构建 孟德尔穷尽分层（MES） 体系，实现重复检测、系谱验证、分级评价、种质挖掘及遗传兼容性分析。系统不预设遗传模型，仅依赖孟德尔分离定律的布尔排除逻辑，确保分析客观可重复，计算复杂度为 O(N²L)（N为种质数，L为标记位点数），适用于种质库快速预筛。当前版本针对SSR标记优化，SNP版本待后续升级。
