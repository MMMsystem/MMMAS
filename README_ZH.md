...
MMMAS（孟德尔错配矩阵分析系统）是一款基于布尔孟德尔排除法则的种质库预筛工具，核心创新在于将成对排除逻辑拓展为 N×N对称错配矩阵，提出 Mmin（最小错配数）、Mavg（平均错配数）和 Mzmp（零错配伙伴数）等核心指标，并构建孟德尔穷尽分层（MES）体系，实现重复检测、系谱验证、分级评价、种质挖掘及遗传兼容性分析。系统不预设遗传模型，仅依赖孟德尔分离定律的布尔排除逻辑，确保分析客观可重复，计算复杂度为 O(N²L)（N为种质数，L为标记位点数），适用于种质库快速预筛。当前版本针对SSR标记优化，SNP版本待后续升级。
...
| 样本数 | ≥ 2 |
| 位点数 | ≥ 5（推荐 ≥ 11） |
| 缺失数据 | SSR 标记：缺失数据允许存在，可用以下方式表示：空值（None/''）、na/nan/NA/NaN、null/NULL、0、./.、-、.、-9、-1、000、999、9999。缺失位点在计算孟德尔错配矩阵时不计入有效比较位点数（Lij），不影响错配计数；SNP 标记（0/1/2 编码）：缺失数据允许存在，可用以下方式表示：空值（None/''）、na/nan/NA/NaN、null/NULL、./.、-、.、-9、-1、000、999、9999。注意：0 为有效基因型（纯合参考型），不可作为缺失值使用。缺失位点同样不计入有效比较位点数。|
| 编码方式 | 自动检测 SSR/SNP |
...
Mzmp（零错配伙伴数）高说明该样本在库中有很多疑似亲子关系的品种，可能为：
...
| Dk 级别 | 生物学意义 |
|---|---|
| Dk=0 | 该级别的样本在库中存在至少1个样本与其零错配（所有位点均符合孟德尔亲子关系传递），不能排除二者为亲子关系 |
| Dk=1 | 该级别的样本与库中所有其它样本至少存在1个错配位点（1个位点不符合孟德尔亲子关系传递） |
| Dk≥2 | 该级别的样本与库中所有其它样本至少存在K个错配位点（K个位点不符合孟德尔亲子关系传递） |
