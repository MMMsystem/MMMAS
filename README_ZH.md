...
| 样本数 | ≥ 2 |
| 位点数 | ≥ 5（推荐 ≥ 11） |
| 缺失数据 | SSR 标记：缺失数据允许存在，可用以下方式表示：空值（None/''）、na/nan/NA/NaN、null/NULL、0、./.、-、.、-9、-1、000、999、9999。缺失位点在计算孟德尔错配矩阵时不计入有效比较位点数（Lij），不影响错配计数。
SNP 标记（0/1/2 编码）：缺失数据允许存在，可用以下方式表示：空值（None/''）、na/nan/NA/NaN、null/NULL、./.、-、.、-9、-1、000、999、9999。注意：0 为有效基因型（纯合参考型），不可作为缺失值使用。缺失位点同样不计入有效比较位点数。|
| 编码方式 | 自动检测 SSR/SNP |
